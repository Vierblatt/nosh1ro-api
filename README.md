...
| 数据库 | MySQL 8.0 / SQLite（可插拔，一行配置切换） |
...
- **可插拔数据库**：同一份代码支持 MySQL 和 SQLite，`DB_TYPE` 环境变量切换，零代码改动
...
│   ├── store/store.go                  # 数据访问层（MySQL/SQLite 双驱动）
...
```bash
DB_TYPE=sqlite DB_DSN=blog.db go run ./cmd/api/
```
...
docker run -p 8080:8080 \
  -e DB_TYPE=mysql \
  -e DB_DSN="root:password@tcp(host.docker.internal:3306)/blog?charset=utf8mb4&parseTime=true&loc=Local" \
